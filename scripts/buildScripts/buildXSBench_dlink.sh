...
for src in "${SOURCES[@]}"; do
    ...
    # 4. Instrument the flattened IR
    echo "Running DevicePass instrumentation..."
    $LLVM_DIR/bin/opt -load-pass-plugin=./build/lib/device/DevicePass.so \
        -passes="fp-exception" -count-total=false ${base}_device_preopt.bc \
...
for src in "${SOURCES[@]}"; do
    ...
    $LLVM_DIR/bin/clang++ -O1 -c ${base}_instrumented_host.bc \
        -o ${base}.o
